package offer.compass.amazondeal.constants;

public class PriceHistoryConstants {

    public static final String URL = "https://pricehistory.in/";
    public static final String LOWEST_PRICE_ID = "lowestPrice";
    public static final String HIGHEST_PRICE_ID = "highestPrice";
    public static final String CURRENT_PRICE_ID = "currentPrice";
    public static final String DROP_CHNACES_ID = "dropChances";
    public static final String PRODUCT_NAME_ID = "name";
    public static final String PRICE_HISTORY_URL_XPATH = "/html/body/div[2]/div/div/div/div[2]/div[1]/input";
    public static final String PRICE_HISTORY_TRACK_SEARCH_ID = "trackPriceBtn";
    public static final String GRAPH_CLASS = "highcharts-background";
    public static String PATH_TO_SAVE_SS = "D:\\OfferCompass\\images\\price-history\\";
}
